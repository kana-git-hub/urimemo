...
### Main（メイン）— 初期画面
画面下部の独自タブバーで「レジ」「集計」「メモ」を切替え、フローティングの「登録」ボタンから商品登録へ遷移します。
...
**メモタブ（寄贈・交換の記録）**
- 「◯◯さんに新刊1冊寄贈」「△△と既刊を交換」のような自由記述メモを日時付きで記録
- メモのタップで編集、モーダルから削除
- 売上カウントとは独立した記録（売上には影響しません）
...
`AsyncStorage` に、以下のデータを JSON 文字列で保存します。
